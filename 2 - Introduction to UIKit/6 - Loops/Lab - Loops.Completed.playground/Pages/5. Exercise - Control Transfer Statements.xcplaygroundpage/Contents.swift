...
for (index, letter) in alphabet.enumerated() {
    ...
    var stateCaptials = [String: String]()
    stateCaptials["Utah"] = "Salt Lake City"
    stateCaptials["Nevada"] = "Carson City"
    stateCaptials["Oregon"] = "Portland"
    for state in stateCaptials {
        print("\(state.key) has a state capital of \(state.value)")
        if state.key == "Utah" {
