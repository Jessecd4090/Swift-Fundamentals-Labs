...
// It will create an error because you are trying to print the value of foo using string interpolation, but it only exists in the for loop
...
// Unlike the example above, in this example, we have stated the variable x to exist globally instead of just within the for loop, we can then reference/modify that inside the loop, not the other way around (from loop to global use) foo is defined in the for loop, and thus can't be used outside
...
greeting(greeting: "", name: "Jestin")
greeting(greeting: nil, name: "Jestin")
